...
from testdata.getpath import GetTestReport, GetConfigFile
...
<table width="800" border="0" cellspacing="0" cellpadding="4">
    <tr>
        <td bgcolor="#CECFAD" height="20" style="font-size:20px">Hi 附件为接口自动化测试报告，请查阅<a href="http://127.0.0.0:8000"> 更多内容>></a></td>
    </tr><br><br>
    <tr>
        <td bgcolor="#EFEBDE" height="100" style="font-size:23px">
            1)测试用例总数：<font color=red>{case_num}</font><br><br>
            2)通过用例总数：<font color=red>{pass_num}</font><br><br>
            3)失败用例总数：<font color=red>{fail_num}</font><br><br>
            4)通过率：<font color=red>{passrate}</font><br><br>
            5)接口请求地址：<font color=red>{testlink}</font><br><br>
            6)测试时间：<font color=red>{test_time}</font><br><br>
                        详细内容查看附件：<font color=red>{attach_name}</font><br><br>
        </td>
    </tr>
"""
try:
    ...
    mail_content = msg.format(case_num=len(TestReport), pass_num=hpassnum, fail_num=len(TestReport) - hpassnum,
                              passrate="{}%".format(hpassnum / len(TestReport) * 100), testlink=testurl, test_time=timenow,
                              attach_name=ReportPath.split('/')[-1])
    mail_title = '【测试报告】接口测试报告'
    # attachment = set(['./testreport/2018-11-18-23-41-55-TestReport.xls', ])
    attachment = set([ReportPath, ])
